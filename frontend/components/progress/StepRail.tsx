...
// project on top, then connected dots with stage labels.
  ...
  return (
    <div>
      ...
      {RAIL_STAGES.map((stage, index) => {
        ...
        const isLast = index === RAIL_STAGES.length - 1
        return (
          <div key={stage.key} className="flex items-stretch gap-2.5">
            <div className="flex flex-col items-center">
              <div className={`flex h-5 w-5 items-center justify-center rounded-full border-2 text-[10px] font-bold transition-all duration-300 ${isCompleted ? "border-indigo-600 bg-indigo-600 text-white" : "border-gray-300 bg-white text-gray-400"}`}>
                {isCompleted ? "✓" : index + 1}
              </div>
              {!isLast && <div className={`w-0.5 flex-1 transition-colors duration-300 ${isCompleted ? "bg-indigo-300" : "bg-gray-200"}`} />}
            </div>
            <p className={`pb-4 text-xs leading-tight transition-colors duration-300 ${isCompleted ? "font-medium text-indigo-700" : "text-gray-400"}`}>{stage.label}</p>
          </div>
        )
      })}
    </div>
